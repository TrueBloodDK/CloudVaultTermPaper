...
2. **Авторизация** — RBAC: `system_admin` / `security_admin` / `user`, отделы, руководители отделов и объектные права на файлы/папки
...
В проекте реализована гибкая модель разграничения доступа:

- глобальные роли разделяют администратора системы, администратора безопасности и обычных пользователей;
- пользователи могут состоять в нескольких отделах как руководители или сотрудники;
- права можно выдавать конкретным пользователям, отделам и ролям внутри отдела;
- права работают для файлов и папок, а права папок наследуются дочерними папками и файлами;
- `deny`-правила позволяют делать точечные исключения из общих разрешений;
- выдача прав ограничена делегированием: пользователь не может передать действие, которого нет у него самого;
